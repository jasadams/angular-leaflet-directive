(function () {
        ...
        link: function (scope, element, attrs, controller) {
          ...
          controller.getMap().then(function (map) {
            ...
            if (isDefined(controls.zoom)) {
              var zoomOptions = { position: 'topleft' };
  ...
  angular.module('leaflet-directive').directive('dyngeojson', [
    '$log',
    '$rootScope',
    '$timeout',
    'leafletData',
      ...
      return {
        ...
        link: function (scope, element, attrs, controller) {
          ...
          controller.getMap().then(function (map) {
            ...
            var resetStyleOnMouseout = dyngeojson.resetStyleOnMouseout, onEachFeatureSupp = dyngeojson.onEachFeature;
  ...
  angular.module('leaflet-directive').directive('markerCluster', [
    '$log',
    '$rootScope',
    'leafletData',
    'leafletHelpers',
          ...
          controller.getMap().then(function (map) {
            ...
            markerCluster.markers.forEach(function (geoJsonObj) {
              var newMarker = new L.Marker([
                  geoJsonObj.geometry.coordinates[1],
        ...
        link: function (scope, element, attrs, controller) {
          ...
          controller.getMap().then(function (map) {
            ...
            function long2x(lon, zoom) {
              return Math.floor((lon + 180) / 360 * Math.pow(2, zoom) * 256 + 0.5);
            }
            function lat2y(lat, zoom) {
              return Math.floor((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom) * 256 + 0.5);
            }
            function moveClick() {
              setMode('move');
            }
